...
> -- Apakah kita bisa menambahkan pada RobotState informasi tentang Energy? Misalnya robot
...
> -- Bisa, kita menambah field Energy pada tipe RobotState, 

> -- Fungsi tambahan:

> decreaseEnergy :: Int -> Robot ()
> decreaseEnergy x = updateState (\s -> s {energy = (energy s) - x})

> requireEnergy :: Int -> Robot Bool
> requireEnergy x = queryState (\s -> enregi s >= x)

> -- Fungsi tersebut perlu dipanggil di setiap gerakan misal move atau pendown,
> -- untuk mengurangi energi dan mengecek apakah 
> -- energy yang dibutuh cukup dengan cond (requireEnergy x) ...

> -- Tipe dari mapM_ adalah
> -- mapM_ :: (Foldable t, Monad m) => (a -> m b) -> t a -> m ()
> -- mapM_ adalah fungsi mapM yang dijalankan untuk side-effectnya saja
> -- karena jika kita mapM dengan fungsi a -> m () maka return typenya
> -- akan menjadi m (t ()). t () merupakan tipe yang bisa diabaikan sehinga
> -- lebih baik dibuat mejadi m () dengan mapM
